@props(['name' => null, 'value' => null, 'inputType' => 'input', 'hourCycle' => 'auto', 'seconds' => false, 'minuteStep' => 1, 'secondStep' => 1, 'min' => null, 'max' => null, 'disabled' => false, 'id' => null, 'part' => null, 'fullWidth' => false, 'variant' => null, 'color' => 'default', 'size' => 'md'])
@php
...
@endphp
<div data-slot="time-field" data-time-config='@json($config)' data-variant="{{ $variant }}" @if($hasExplicitVariant) data-group-variant="explicit" @endif data-color="{{ $color }}" data-size="{{ $size }}" @if($id) id="{{ $id }}" @endif {{ $attributes->class(['app-time-field', 'app-time-field-'.$variant, 'app-time-field-'.$size, 'app-color-'.$color, 'app-time-field-full' => $fullWidth]) }}>
