...
src와 alt를 지정해 이미지를 표시합니다. alt에는 이미지의 의미를 설명하는 문장을 작성합니다.

```blade preview name="image-default"
<x-image
...
같은 이미지를 여러 비율로 표시합니다.

```blade preview name="image-ratios"
...
cover와 contain 표시 방식을 선택합니다.

```blade preview name="image-fit"
...
콘텐츠 이미지와 프로필 이미지에 맞춰 radius를 조절합니다.

```blade preview name="image-rounded"
