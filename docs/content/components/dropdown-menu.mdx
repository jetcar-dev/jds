...
parts: [dropdown-menu, dropdown-menu-trigger, dropdown-menu-content, dropdown-menu-item, dropdown-menu-checkbox-item, dropdown-menu-radio-group, dropdown-menu-radio-item, dropdown-menu-separator, dropdown-menu-sub, dropdown-menu-sub-trigger, dropdown-menu-sub-content]
---

## 기본 사용법

트리거에는 원하는 버튼을 넣고, 메뉴 내용에는 필요한 항목을 순서대로 배치합니다.

```blade preview name="dropdown-menu-default"
<x-dropdown-menu>
...
메뉴 항목도 공통 variant, color, size 값을 사용합니다.

```blade preview name="dropdown-menu-appearance"
<x-dropdown-menu>
...
체크 항목과 라디오 항목은 각각 독립된 선택 상태를 가집니다.

```blade preview name="dropdown-menu-states"
<x-dropdown-menu>
    <x-dropdown-menu-trigger><x-button variant="bordered">보기</x-button></x-dropdown-menu-trigger>
    <x-dropdown-menu-content>
        <x-dropdown-menu-checkbox-item checked>상태 표시</x-dropdown-menu-checkbox-item>
...
하위 메뉴 안에 트리거와 내용을 넣으면 마우스를 올리거나 키보드로 이동할 때 메뉴가 열립니다.

```blade preview name="dropdown-menu-advanced"
<x-dropdown-menu>
