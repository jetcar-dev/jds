...
각 항목에 제목과 본문을 넣어 구성합니다. 본문에는 HTML과 다른 JDS 컴포넌트를 자유롭게 배치할 수 있습니다.

```blade preview name="accordion-default"
...
type="multiple"을 사용하면 여러 영역을 동시에 열 수 있습니다. value 배열로 처음 열어 둘 항목을 지정합니다.
...
제목 영역에서 아이콘의 형태와 위치, 비활성 상태를 지정할 수 있습니다. 열림 상태에 따라 아이콘이 자연스럽게 전환됩니다.

```blade preview name="accordion-advanced"
<x-accordion>
...
긴 등록 화면을 의미 있는 구역으로 나누고 각 본문 안에 필요한 입력 요소를 배치합니다.

```blade preview name="accordion-form-sections"
    ...
    <x-accordion-item value="vehicle">
        ...
        <x-accordion-content>
            <div class="jds-example-grid">
                <x-field><x-label for="plate">차량번호</x-label><x-input id="plate" name="plate" full-width /></x-field>
                <x-field><x-label for="model">차종</x-label><x-input id="model" name="model" full-width /></x-field>
            </div>
        </x-accordion-content>
    </x-accordion-item>
    <x-accordion-item value="owner">
        <x-accordion-trigger>소유자 정보</x-accordion-trigger>
        <x-accordion-content><x-input name="owner" placeholder="소유자명" full-width /></x-accordion-content>
    </x-accordion-item>
</x-accordion>
```
