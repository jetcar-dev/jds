...
마지막 항목은 자동으로 현재 페이지가 됩니다.

```blade preview name="breadcrumb-default"
<x-breadcrumb :items="['홈' => '/', '회원' => '/members', '회원 상세']" />
```

## 구분 기호 바꾸기

separator 속성으로 구분 문자를 바꿉니다.

```blade preview name="breadcrumb-separator"
<x-breadcrumb :items="['문서' => '/docs', '설치']" separator="/" />
```

## ERP 화면 경로

깊은 업무 화면에서는 상위 메뉴부터 현재 레코드까지 경로를 전달합니다.

```blade preview name="breadcrumb-erp-path"
<x-breadcrumb
    aria-label="차량 상세 경로"
    :items="[
        '대시보드' => '/',
...
배열 대신 하위 컴포넌트를 직접 조합하면 말줄임이나 아이콘 같은 특수 경로도 만들 수 있습니다.

```blade preview name="breadcrumb-custom-content"
