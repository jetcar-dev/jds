(function (window) {
    ...
    AppUI.collection = (root, options = {}) => {
        ...
        const selectionBehavior = options.selectionBehavior || 'toggle'
        const disallowEmptySelection = options.disallowEmptySelection === true
        ...
        const select = (item, notify = true) => {
            if (!item || selectionMode === 'none' || item.dataset.readonly === 'true') return
            ...
            else if (selectionBehavior === 'replace') items().forEach(value => { value.dataset.selected = value === item ? 'true' : 'false'; value.setAttribute('aria-selected', value === item ? 'true' : 'false') })
            else if (next || !disallowEmptySelection || selected().length > 1) { item.dataset.selected = next ? 'true' : 'false'; item.setAttribute('aria-selected', next ? 'true' : 'false') }
        ...
        const prepare = () => {
            ...
            items().forEach((item, index) => {
                ...
                if (root.dataset.virtualize === 'true') {
                    ...
                    item.style.containIntrinsicSize = `0 ${Math.max(1, Number(root.dataset.itemHeight || 40))}px`
                }
        ...
        root.addEventListener('keydown', event => {
            ...
            if (event.key === 'ArrowDown' || event.key === 'ArrowRight') { event.preventDefault(); focus(enabled[shouldFocusWrap ? (current + 1 + enabled.length) % enabled.length : Math.min(enabled.length - 1, current + 1)]) }
            else if (event.key === 'ArrowUp' || event.key === 'ArrowLeft') { event.preventDefault(); focus(enabled[shouldFocusWrap ? (current - 1 + enabled.length) % enabled.length : Math.max(0, current - 1)]) }
